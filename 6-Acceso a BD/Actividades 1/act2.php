        ...
        <div class="abs-center">
            <div class="container">
                ...
                <div class="row border">
                    <form method="post" action="#" class=" p-3 form">
                        <div class="form-group row">
                            <label for="Ecuacion">Titulos Dos palabras</label>
                            <input type="text" class="form-control col" name="titulo2">
                            <button type="submit" name="buscar2" class="btn btn-primary col">Buscar</button>
                        </div>
                    </form>
                    <?php
                    if (isset($_POST['buscar2'])) {
                        $titulo2 = $_POST['titulo2'];
                        $palabras = explode(" ",$titulo2);
                        foreach ($palabras as $palabra) {
                            if ($resultado = mysqli_query($enlace, "SELECT * FROM hoja1 WHERE Nombre_libro LIKE '%$palabra%' ")) {
                                while ($fila = mysqli_fetch_row($resultado)) {
                    ?>
                                    <hr>
                                    <div class="row">
